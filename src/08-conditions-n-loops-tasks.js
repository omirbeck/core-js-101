...
function doRectanglesOverlap(rect1, rect2) {
  return rect1.left + rect1.width > rect2.left && rect1.top + rect1.height > rect2.top
        && rect2.left + rect2.width > rect1.left && rect2.top + rect2.height > rect1.top;
}
...
function isCreditCardNumber(ccn) {
  ...
  arr.forEach((_item, i) => {
    ...
    else res.push(arr[i] * 2);
  });
  // eslint-disable-next-line no-shadow
  const sum = res.reduce((sum, current) => sum + current, 0);
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  if (num < 9) return num;
  const arr = String(num).split('').reduce((item1, item2) => Number(item1) + Number(item2));
